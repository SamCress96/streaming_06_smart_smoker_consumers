"""
    This program receives messages from the Food1 queue the RabbitMQ server.
    The program produces an alert when food temperature has stalled (hasn't increased by at least 1 degree in 20)  
...
def food1_callback(ch, method, properties, body):
    ...
    food1_message1_split = food1_message1.split(", ")
    # change to float 
    food1_message1_temp = float(food1_message1_split[1])
...
def main(host: str, queue: str):
    ...
    try:
        ...
        channel.queue_declare(queue=queueF1, durable=True)
        ...
        channel.basic_consume(queue=queueF1, on_message_callback=food1_callback, auto_ack=False)
